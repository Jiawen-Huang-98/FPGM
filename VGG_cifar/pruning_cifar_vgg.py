...
args = parser.parse_args(['../datasets/CIFAR10/cifar.python', '--dataset', 'cifar10','--test-batch-size','16',
                          '--save_path', './logs/best_model_test/', '--epochs', '200', '--lr', '0.1',
                          '--batch-size', '16', '--layer_end', '57',])
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            model = models.vgg_cifar(dataset = 'cifar10', depth = 16)
        ...
        if batch_idx % args.log_interval == 0:
            print_log('Train Epoch: {} [{}/{} ({:.1f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                       100. * batch_idx / len(train_loader), loss.data.item()), log)
